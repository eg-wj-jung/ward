...
list_a001s_rds <- list.files(file.path(data_dir, filing_A001_kosdaq_rds_dir), full.names = T)
...
for(n_a001s in 1:end_a001){
  ...
  a001_kosdaq_zip_dir <- paste0(data_dir, biz_report_listed_kosdaq_zip_dir, "corp_code_", corp_code_a001)
  dir_create(a001_kosdaq_zip_dir)
  ...
  for(n_a001 in 1:nrow(list_a001s)){
    ...
    path_a001 <- paste(a001_kosdaq_zip_dir, "/", list_a001s[n_a001, "year"], "_", recept_no, ".zip", sep = "")
...
list_a001s_kosdaq_zip <- list.files(file.path(data_dir, biz_report_listed_kosdaq_zip_dir), recursive = TRUE, full.names = TRUE)

list_a001s <- data.frame(path = list_a001s_kosdaq_zip, 
                         corp_code = str_extract(list_a001s_kosdaq_zip, "(?<=corp_code_)[0-9]{1,8}"), 
                         fiscal_year = str_extract_all(list_a001s_kosdaq_zip, "(?<=/)[0-9]{1,4}", simplify = T)[, 7])
...
for(n_corp in 1:end_a001s){
  ...
  for (n_zip in 1:n_a001s_sub) {
    ...
    path_dir <- paste0(data_dir, biz_report_listed_kosdaq_xml_dir, "corp_code_", corp_code, "/", list_a001s_sub$fiscal_year[n_zip])
    dir_create(path_dir)
