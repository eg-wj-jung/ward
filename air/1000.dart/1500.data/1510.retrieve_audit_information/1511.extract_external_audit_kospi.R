...
indv_df_external_audit_kospi <- data.frame()
gross_external_audit_kospi <- data.frame()
...
for(n_corp in 1:end_corp) {
  ...
  for(n_file in 1:nrow(df_list_ar_xml_sub)) {
    ...
    indv_df_external_audit_kospi <- bind_rows(indv_df_external_audit_kospi, df_external_audit)
    df_external_audit <- data.frame()
  }
  
  gross_external_audit_kospi <- bind_rows(gross_external_audit_kospi, indv_df_external_audit_kospi)
  indv_df_external_audit_kospi <- data.frame()
}

#### write for rds #####
saveRDS(gross_external_audit_kospi, "~/data/raw/10000.dart/15000.data/ex_audit/external_audit_kospi.rds")
